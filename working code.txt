...
let adminAwaitingCodeAmount = false;
let adminCodeAmount = null;
let adminAwaitingCodesInput = false;

let adminAwaitingViewAmount = false;
let adminAwaitingRemoveAmount = false;
let adminRemoveAmount = null;
let adminAwaitingRemoveCodes = false;

let adminBroadcastAwaitingMedia = false;
let adminBroadcastAwaitingText = false;
let adminBroadcastAwaitingButtons = false;
...
function resetAdminState() {
    ...
    adminAwaitingCodeAmount = false;
    adminCodeAmount = null;
    adminAwaitingCodesInput = false;
    adminAwaitingViewAmount = false;
    adminAwaitingRemoveAmount = false;
    adminRemoveAmount = null;
    adminAwaitingRemoveCodes = false;
}

function resetBroadcastState() {
    adminBroadcastAwaitingMedia = false;
    adminBroadcastAwaitingText = false;
    adminBroadcastAwaitingButtons = false;
    broadcastMedia = null;
    broadcastText = '';
    broadcastButtons = [];
}
...
bot.on('callback_query', async (callbackQuery) => {
    ...
    if (userId === ADMIN_CHAT_ID) {
        if (data === 'broadcast_skip_buttons') {
            ...
            const hasMedia = broadcastMedia !== null;
            const hasText = broadcastText && broadcastText.trim();
            const hasButtons = broadcastButtons.length > 0;
            ...
            const opts = {
                parse_mode: 'Markdown',
                reply_markup: hasButtons ? { inline_keyboard: [broadcastButtons] } : undefined
            };

            if (hasMedia) {
                const { type, fileId } = broadcastMedia;
                if (type === 'photo') await bot.sendPhoto(chatId, fileId, { caption: broadcastText, ...opts });
                if (type === 'video') await bot.sendVideo(chatId, fileId, { caption: broadcastText, ...opts });
                if (type === 'animation') await bot.sendAnimation(chatId, fileId, { caption: broadcastText, ...opts });
            } else if (hasText) {
                await bot.sendMessage(chatId, broadcastText, opts);
            }
        ...
        if (data === 'broadcast_edit') {
            adminBroadcastAwaitingMedia = true;
            adminBroadcastAwaitingText = false;
            adminBroadcastAwaitingButtons = false;
        ...
        if (data === 'broadcast_publish') {
            for (const uid of Object.keys(users)) {
                const opts = {
                    parse_mode: 'Markdown',
                    reply_markup: broadcastButtons.length ? { inline_keyboard: broadcastButtons } : undefined
                };
                ...
                try {
                    if (broadcastMedia) {
                        const { type, fileId } = broadcastMedia;
                        if (type === 'photo') await bot.sendPhoto(uid, fileId, { caption: broadcastText, ...opts });
                        if (type === 'video') await bot.sendVideo(uid, fileId, { caption: broadcastText, ...opts });
                        if (type === 'animation') await bot.sendAnimation(uid, fileId, { caption: broadcastText, ...opts });
                    } else {
                        await bot.sendMessage(uid, broadcastText, opts);
                    }
                } catch (err) {
    ...
    if (data === 'broadcast_skip_media') {
        adminBroadcastAwaitingMedia = false;
        adminBroadcastAwaitingText = true;
    ...
    if (data === 'broadcast_skip_text') {
        adminBroadcastAwaitingText = false;
        adminBroadcastAwaitingButtons = true;
...
bot.onText(/\/cancel/, (msg) => {
    ...
    if (adminBroadcastAwaitingMedia || adminBroadcastAwaitingText || adminBroadcastAwaitingButtons) {
        resetBroadcastState();
...
async function handleAdminBroadcastFlow(msg, chatId, userId, text) {
    ...
    if (adminBroadcastAwaitingMedia) {
        if (msg.photo || msg.video || msg.animation) {
            broadcastMedia = msg.photo ? { type: 'photo', fileId: msg.photo[msg.photo.length - 1].file_id } :
    ...
    if (adminBroadcastAwaitingButtons) {
        broadcastButtons = parseButtons(text);
        adminBroadcastAwaitingButtons = false;
...
function showBroadcastPreview(chatId) {
    ...
    if (broadcastMedia) {
        const { type, fileId } = broadcastMedia;
        if (type === 'photo') bot.sendPhoto(chatId, fileId, { caption: broadcastText, ...opts });
        if (type === 'video') bot.sendVideo(chatId, fileId, { caption: broadcastText, ...opts });
        if (type === 'animation') bot.sendAnimation(chatId, fileId, { caption: broadcastText, ...opts });
    } else {
        bot.sendMessage(chatId, broadcastText, opts);
    }
...
async function handleAdminCodeFlow(msg, chatId, userId, text) {
    ...
    if (adminAwaitingCodeAmount && userId === ADMIN_CHAT_ID) {
        const amountMap = {
            '₹15 (3⭐)': 3,
            '₹25 (5⭐)': 5,
            '₹50 (10⭐)': 10,
        ...
        if (amountMap[text]) {
            adminCodeAmount = amountMap[text];
            adminAwaitingCodeAmount = false;
            adminAwaitingCodesInput = true;

            bot.sendMessage(chatId,
                `✅ You selected *₹${adminCodeAmount}* redemption codes.\n\n` +
    ...
    // Handle codes input
    if (adminAwaitingCodesInput && userId === ADMIN_CHAT_ID) {
        ...
        const codesFile = path.join(__dirname, 'data', 'codes', `${adminCodeAmount}.json`);
        ...
        bot.sendMessage(chatId, `✅ Successfully added *${codes.length}* codes to the ₹${adminCodeAmount} file.`, { parse_mode: 'Markdown' });
    ...
    if (adminAwaitingViewAmount && userId === ADMIN_CHAT_ID) {
        const amountMap = {
            '₹15 (3⭐)': 3,
            '₹25 (5⭐)': 5,
            '₹50 (10⭐)': 10,
    ...
    if (adminAwaitingRemoveAmount && userId === ADMIN_CHAT_ID) {
        const amountMap = {
            '₹15 (3⭐)': 3,
            '₹25 (5⭐)': 5,
            '₹50 (10⭐)': 10,
        ...
        if (amountMap[text]) {
            adminRemoveAmount = amountMap[text];
            adminAwaitingRemoveAmount = false;
            adminAwaitingRemoveCodes = true;

            bot.sendMessage(chatId, `✅ You selected ₹${adminRemoveAmount} codes.\n\n💡 Please send the codes to remove, separated by commas.`);
            return true;
        } else if (text === '⬅️ BACK') {
            resetAdminState();
            sendAdminMenu(chatId);
    ...
    if (adminAwaitingRemoveCodes && userId === ADMIN_CHAT_ID) {
        ...
        const codesFile = path.join(__dirname, 'data', 'codes', `${adminRemoveAmount}.json`);

        if (!fs.existsSync(codesFile)) {
            bot.sendMessage(chatId, `⚠️ No codes file found for ₹${adminRemoveAmount}.`);
        } else {
            ...
            bot.sendMessage(chatId, `✅ Removed *${removedCount}* codes from ₹${adminRemoveAmount} file.`, { parse_mode: 'Markdown' });

            // Show remaining codes
            if (existingCodes.length === 0) {
                bot.sendMessage(chatId, `ℹ️ No remaining codes for ₹${adminRemoveAmount}.`);
            } else {
                bot.sendMessage(chatId,
                    `📄 *Remaining Codes for ₹${adminRemoveAmount}:*\n\n` +
    ...
    if (userId !== ADMIN_CHAT_ID) return false;
      ...
      switch (text) {
        ...
        case '🧾 View Avaailable Codes 🧾':
            ...
            adminAwaitingViewAmount = true;
            return true;

        case '🗑 Remove Codes':
            resetAdminState();  // Safe to reset first
            adminAwaitingRemoveAmount = true;  // Set correct state
...
async function handleAdminUserLookup(chatId, userId, text) {
    if (!adminAwaitingUserId || userId !== ADMIN_CHAT_ID) return false;
